...
proto:
	rm -rf /build/mytool/go/src/logic/proto /build/react/app/src/proto
	# latestが更新されてなくて古いままなので0.4.0を指定
	docker run -v "`pwd`:/build" -w /build --rm znly/protoc:0.4.0 -I proto --go_out=plugins=grpc:/build/mytool/go/src/logic/proto --js_out=import_style=commonjs,binary:/build/react/app/src/proto --grpc-web_out=import_style=commonjs,mode=grpcweb:/build/react/app/src/proto proto/hello.proto
	...
	# docker run -v "`pwd`:/build" -w /build --rm namely/protoc-all:latest -d proto -l go -o /build/mytool/go/src/logic/proto
